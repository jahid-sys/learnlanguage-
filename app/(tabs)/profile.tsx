...
export default function ProfileScreen() {
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      >
        <Pressable
          style={profileStyles.alertOverlay}
          onPress={() => setAlertModal(prev => ({ ...prev, visible: false }))}
        >
          <Pressable style={[profileStyles.alertContainer, { backgroundColor: colors.card }]} onPress={() => {}}>
            <Text style={profileStyles.alertTitle}>{alertModal.title}</Text>
            {alertModal.message ? <Text style={profileStyles.alertMessage}>{alertModal.message}</Text> : null}
            <View style={profileStyles.alertButtonRow}>
              {alertModal.onConfirm && (
                <TouchableOpacity
                  style={[profileStyles.alertButton, profileStyles.alertCancelButton]}
                  onPress={() => setAlertModal(prev => ({ ...prev, visible: false }))}
                >
                  <Text style={[profileStyles.alertButtonText, { color: colors.text }]}>Cancel</Text>
                </TouchableOpacity>
              )}
              <TouchableOpacity
                style={[profileStyles.alertButton, { backgroundColor: colors.primary }]}
                onPress={() => {
                  setAlertModal(prev => ({ ...prev, visible: false }));
                  alertModal.onConfirm?.();
                }}
              >
                <Text style={profileStyles.alertButtonText}>{alertModal.onConfirm ? 'Confirm' : 'OK'}</Text>
              </TouchableOpacity>
            </View>
          </Pressable>
        </Pressable>
